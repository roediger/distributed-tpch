...
order by
  revenue desc;
select
  round(sum(l_extendedprice * l_discount), 5) as revenue
from
  lineitem
where
...
select
  nation,
  o_year,
  round(sum(amount), 1) as sum_profit
from (
  select
    n_name as nation,
    year(o_orderdate) as o_year,
...
order by
  ...
  p_size;
select
  round(sum(l_extendedprice) / 7.0, 5) as avg_yearly
from
  lineitem,
  part
where
